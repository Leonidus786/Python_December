...
if all_divs:
    ...
    output_file_path = 'job_profiles.txt'

    # Open the text file in write mode
    with open(output_file_path, 'w', encoding='utf-8') as file:
        # Writing the top ten job profiles to the file
        count = 0
        for job_profile in job_profiles:
            file.write(job_profile.text + '\n')
            count += 1
            if count == 10:
                break
    print(f"Job profiles saved to {output_file_path}")
else:
